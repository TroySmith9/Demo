...
public class MainClass {
	...
	private static void TestJsonAttribute() {
		...
		System.out.println("Int_att:" + jsonObj.getIntValue("Int_att"));
	...
	private static void testJsonBean() {
		 ...
		 */
		JSONObject jsonStu = JSONObject.parseObject(student.toString());
		System.out.println(jsonStu.toString());
		System.out.println(jsonStu.getJSONArray("hobby"));
		/**
		 * json对象转换成java对象，并获取java对象属性
		 */
		Student stu = JSONObject.parseObject(String.valueOf(jsonStu), Student.class);
...
//		JSONArray jsonArray = JSONArray.ad(list);
...
//		Collection java_collection = JSONArray.toList(new_jsonArray);
