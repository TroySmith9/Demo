...
public class Sort {
    ...
    public static void main(String[] args) {
        ...
        int[] results;
        //冒泡排序
//        results = bubbleSort(numbers);

        //快速排序:在同数量级（O(nlog2n)）的排序方法中平均性能最好的。但若初始序列按关键码有序或基本有序时，快排序反而蜕化为冒泡排序。为改进之，通常以“三者取中法”来选取基准记录，即将排序区间的两个端点与中点三个记录关键码居中的调整为支点记录。快速排序是一个不稳定的排序方法
//        results = quickSort(numbers, 0, numbers.length - 1);

        // 选择排序算法
//        results = selectSort(numbers);
        //堆排序-二叉树选择排序
        results = heapSort(numbers);

        //插入排序
//        results = insertSort(numbers);
        //希尔排序-特殊插入排序
//        results = shellSort(numbers);

//        归并排序算法
//        results = combineSort(numbers, 0, numbers.length - 1);
...
/*            //选择出应该在第i个位置的数
    ...
    /**
     * 希尔排序-特殊插入排序
     * 原理:根据需求，如果你想要结果从大到小排列，它会首先将数组进行分组，然后将较大值移到前面，较小值
     ...
     * @return 输出有序数组
     */
    public static int[] combineSort(int[] nums, int low, int high) {
        int mid = (low + high) / 2;
        if (low < high) {
            // 左边
            combineSort(nums, low, mid);
            // 右边
            combineSort(nums, mid + 1, high);
            // 左右归并
